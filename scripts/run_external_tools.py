...
def run_fastp(fpath_fastp, l_of_short_reads, common_output_dir, num_threads, min_length):
    fastp_output_dir = common_output_dir + 'fastp_output/'
    os.mkdir(fastp_output_dir)
    cmd_fastp = [fpath_fastp]
    if num_threads > 16:
        n_threads = '16'
    else:
...
def run_porechop(fpath_porechop, long_reads, common_output_dir, num_threads):
    porechop_output_dir = common_output_dir + 'porechop_output/'
    os.mkdir(porechop_output_dir)
    cmd_porechop = f'{fpath_porechop} -t {str(num_threads)} -i {long_reads} -o {porechop_output_dir}trimmed.{os.path.basename(long_reads)}'
    run_external_tool(cmd_porechop, common_output_dir)
    return porechop_output_dir


def run_rnaspades(fpath_rnaspades, l_of_short_reads, long_reads, memory_lim, num_threads, common_output_dir):
    rnaspades_output_dir = common_output_dir + 'rnaspades_output/'
    cmd_rna_spades = ['python3', fpath_rnaspades, '-m', str(memory_lim), '-t', str(num_threads), '--nanopore', long_reads, '-o', rnaspades_output_dir]
...
def run_diamond_blastp(fpath_diamond, fpath_db_diamond, common_output_dir, output_postfix_dir,
                       d_of_optional_args, query_fpath, num_threads, output_filename, taxon_list=None):
    ...
    cmd_diamond_blastp = f'{fpath_diamond} blastp --db {fpath_db_diamond} --query {query_fpath} --evalue {evalue} --max-target-seqs {str(max_target_seqs)} --threads {str(num_threads)} --out {diamond_output_dir}{output_filename} --outfmt {outfmt}'
...
def run_transdecoder_longorfs(fpath_transdecoder, common_output_dir, rnaspades_output_dir):
    transdecoder_output_dir = common_output_dir + 'transdecoder_output'
    cmd_transdecoder_longorfs = f'{fpath_transdecoder}/TransDecoder.LongOrfs -t {rnaspades_output_dir}transcripts.fasta --output_dir {transdecoder_output_dir}'
    run_external_tool(cmd_transdecoder_longorfs, common_output_dir)
    return transdecoder_output_dir


def run_transdecoder_predict(fpath_transdecoder, common_output_dir, rnaspades_output_dir,
                             transdecoder_output_dir, diamond_file_for_searching_homologous_prots):
    cmd_transdecoder_predict = f'{fpath_transdecoder}/TransDecoder.Predict -t {rnaspades_output_dir}transcripts.fasta --retain_blastp_hits {diamond_file_for_searching_homologous_prots} --output_dir {transdecoder_output_dir}'
    run_external_tool(cmd_transdecoder_predict, common_output_dir)


def run_cdhit(fpath_cdhit, memory_lim, num_threads, common_output_dir, extraction_after_transdecoder_output_dir,
    ...
    cmd_cdhit_est = f'{fpath_cdhit} -M {str(memory_lim * 1000)} -c {prop_idy_seqs} -G {type_alignment} -aS {subseq_len_matching_frac} -p 1 -g 1 -T {str(num_threads)} -i {extraction_after_transdecoder_output_dir}cds_longest_iso.fasta -o {cdhit_output_dir}clust_cds_longest_iso.fasta'
