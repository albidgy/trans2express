...
def run_porechop(long_reads, common_output_dir, num_threads):
    ...
    cmd_porechop = f'porechop -t {num_threads} -i {long_reads} -o {porechop_output_dir}trimmed.{os.path.basename(long_reads)}'
...
def run_diamond_blastp(fpath_db_diamond, common_output_dir, output_postfix_dir,
    ...
    cmd_diamond_blastp = f'diamond blastp --db {fpath_db_diamond} --query {query_fpath} --evalue {evalue} --max-target-seqs {str(max_target_seqs)} --threads {num_threads} --out {diamond_output_dir}{output_filename} --outfmt {outfmt}'
...
def run_transdecoder_longorfs(common_output_dir, rnaspades_output_dir):
    ...
    cmd_transdecoder_longorfs = f'TransDecoder.LongOrfs -t {rnaspades_output_dir}transcripts.fasta --output_dir {transdecoder_output_dir}'
    run_external_tool(cmd_transdecoder_longorfs, common_output_dir)
    return transdecoder_output_dir


def run_transdecoder_predict(common_output_dir, rnaspades_output_dir, transdecoder_output_dir,
                             diamond_file_for_searching_homologous_prots):
    cmd_transdecoder_predict = f'TransDecoder.Predict -t {rnaspades_output_dir}transcripts.fasta --retain_blastp_hits {diamond_file_for_searching_homologous_prots} --output_dir {transdecoder_output_dir}'
...
def run_cdhit(memory_lim, num_threads, common_output_dir, extraction_after_transdecoder_output_dir,
    ...
    cmd_cdhit_est = f'cd-hit-est -M {str(memory_lim * 1000)} -c {prop_idy_seqs} -G {type_alignment} -aS {subseq_len_matching_frac} -p 1 -g 1 -T {str(num_threads)} -i {extraction_after_transdecoder_output_dir}cds_longest_iso.fasta -o {cdhit_output_dir}clust_cds_longest_iso.fasta'
